    ...
    */
    ...
    public void show(Graphics g) {
        ...
        for (Staff staff : sys.staffs) {
            ...
            if (justSawBreak) y1 = staffTop; // remember the start of the new component.
    ...
    public static void fatBar(Graphics g, int x, int y1, int y2, int dx) {g.fillRect(x, y1, dx, y2 - y1);} // dx is how fat it is.

    public static void wings(Graphics g, int x, int y1, int y2, int dx, int dy) { // positive dx will draw a rightHigher one. negative will draw the wing towards left.
    ...
    public void drawDots(Graphics g, int x, int top) { // dots from the top of the single staff
        // assume nLine = 5. (if you're writing a guitar staff which has 6 lines, you will rewrite this method.)
        int H = sys.page.sysFmt.maxH;
