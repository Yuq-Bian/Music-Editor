...
public class Head extends Mass implements Comparable<Head> {
    ...
    public Head(Staff staff, int x, int y) {
        ...
        addReaction(new Reaction("S-S") {
            ...
            public int bid(Gesture g) {
                ...
                if (y1 > y || y2 < y) {
                    return UC.noBid;
                } // heads not in y range reject this gesture
                int hl = Head.this.time.x, hr = hl + W; // left and right side of Head.
                if (x < hl - W || x > hr + W) {
                    return UC.noBid;
                } // must be reasonably close to the head.
                if (x < (hl + W / 2)) {
                    return hl - x;
                }
                if (x > (hr - W / 2)) {
    ...
    public void unStem() {
        if (stem != null) { // no need to unStem a head that is already unStemed
            ...
            if (stem.heads.size() == 0) {
                ...
                stem.setWrongSides();
            } // old stem vasishes if it becomes empty
    ...
    public void joinStem(Stem s) {
        if (stem != null) {
            unStem();
        } // make sure that this head is NOT on some other stem..
